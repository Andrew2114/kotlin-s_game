...
class GameRepositoryImpl(
    ...
    override fun save(game: Game): Game {
        db.executeUpdate(
            """INSERT INTO games 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            listOf<Any>(
                game.id, game.player1Id, game.player1Name,
                game.player2Id, game.player2Name,
                combinationToString(game.secret), game.status.name, game.winnerId ?: ""
            )
        )
        game.moves.forEach { saveMove(game.id, it) }
        return game
    }
    ...
    override fun update(game: Game): Game {
        ...
        game.moves.forEach { move ->
            if (move.moveNumber !in existingMoveNumbers) saveMove(game.id, move)
        }
        return game
    }
    ...
    private fun saveMove(gameId: String, move: Move) {
        db.executeUpdate(
            """INSERT INTO moves 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            listOf<Any>(
                gameId, move.moveNumber, move.playerId, move.playerName,
                combinationToString(move.guess), move.feedback.blackPins, move.feedback.whitePins
            )
        )
    }
    ...
    private fun mapToGame(gameData: Map<String, Any>, movesData: List<Map<String, Any>>): Game {
        return Game(
            ...
            player1Id = gameData["player1_id"]?.toString() ?: "",
            player1Name = gameData["player1_name"]?.toString() ?: "",
            player2Id = gameData["player2_id"]?.toString() ?: "",
            player2Name = gameData["player2_name"]?.toString() ?: "",
            secret = stringToCombination(gameData["secret"]?.toString() ?: ""),
            moves = movesData.map { mapToMove(it) },
            status = try {
                GameStatus.valueOf(gameData["status"]?.toString() ?: "IN_PROGRESS")
            } catch (e: Exception) { GameStatus.IN_PROGRESS },
            winnerId = gameData["winner_id"]?.toString()?.takeIf { it.isNotEmpty() }
        )
    }

    private fun mapToMove(moveData: Map<String, Any>): Move {
        return Move(
            moveNumber = (moveData["move_number"] as? Number)?.toInt() ?: 0,
            guess = stringToCombination(moveData["guess"]?.toString() ?: ""),
            feedback = Feedback(
                (moveData["black_pins"] as? Number)?.toInt() ?: 0,
                (moveData["white_pins"] as? Number)?.toInt() ?: 0
            ),
            playerId = moveData["player_id"]?.toString() ?: "",
            playerName = moveData["player_name"]?.toString() ?: ""
        )
    }

    private fun combinationToString(combination: Combination): String =
        combination.colors.joinToString(",") { it.name }

    private fun stringToCombination(str: String): Combination {
        if (str.isBlank()) return Combination(emptyList())
        return Combination(str.split(",").map { Color.valueOf(it.trim()) })
    }
}